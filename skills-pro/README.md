...
기획서 6장 유료 축(발송 규모·자동화·성과추적)과 요금제 기능표에 대응한다
(4종은 6.3 직접 매핑, interview-prep은 Solo '답장 응대'의 고도화이자 9장 3차 캘린더 인터뷰 예약의 선행 구현).
...
1. **PII 비노출** — 기자 실명·이메일·연락처는 화면/리포트/캘린더 이벤트 **제목·설명**에 절대
   출력하지 않는다. 익명 코드(`기자 #A3F1`) + 매체 + beat로만. 실명·이메일은 **실제 발송/초대
   시점**에 Gmail 수신자(To)·본문 인사말과 캘린더 참석자 필드에만 주입.
...
GPT/Gemini에는 본문을 Instructions로 붙여넣는다. Gmail·Google Calendar MCP 연결 시
**초안 생성→라벨링→캘린더 리마인더·성과 집계까지** 자동화된다
(실제 발송은 항상 사용자가 Gmail 임시보관함에서 직접 — 자동 발송 도구는 없다).
