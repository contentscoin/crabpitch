...
- 동일 기자 7일 내 재발송 금지
- 발송은 **사용자 본인 메일 계정(BYO-Email)** 로 — SMTP 직접 발송(전 플랜) 또는
  Gmail 초안 생성(**Agency 전용**). 전송 수단은 둘이지만 **게이트는 하나**입니다(파일럿
  승인·수신거부 재대조·7일 쿨다운·표현 규정·캠페인당 상한·월 한도).
  설정 → `skills-public/docs/MAIL-SETUP.md`
