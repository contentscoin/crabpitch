...
| 스킬 | 하는 일 | 트리거 예시 | MCP 플랜 |
|---|---|---|---|
| `press-release-writer` | 5W1H·역피라미드 보도자료 + 헤드라인 3안 | "보도자료 써줘", "이 소식 알리고 싶어" | Free |
| `media-kit-builder` | 인터뷰형 미디어킷(보일러플레이트·팩트시트·인용문) | "미디어킷 만들어줘", "프레스킷" | Solo+ |
| `journalist-outreach` | 주제-기자 매칭 + 개인화 메일 + Gmail '언론홍보' 라벨 초안 | "기자에게 뿌려줘", "언론 배포" | Solo+ |
| `reply-handler` | 회신 7유형 분류·응대 + '언론홍보' 라벨 모니터링 | "기자 답장", "회신 왔어" | Solo+ |

**「MCP 플랜」은 *채팅에서 크랩피치 도구를 부를 때*의 조건입니다.** 스킬 마크다운 자체는 어느
플랜이든 그대로 쓸 수 있고, 매칭·발송·회신·미디어킷은 **웹앱에서 Free로도 이용**할 수 있습니다.
...
2. **+ CrabPitch MCP** (`cp_mcp_…` 키) — 채팅에서 크랩피치 도구를 직접 호출 → [`docs/MCP-SETUP.md`](./docs/MCP-SETUP.md)
   - **Free**: `crabpitch_press_guide`(보도자료 규범·규칙 검사) · `crabpitch_mail_setup`(발신 메일 설정 안내)
   - **Solo 이상**: + 기자 매칭 · 메일 템플릿 · 회신 분류
   - 잠긴 도구는 채팅에서 **목록에 보이지 않습니다.** 다만 그 기능들은
     **웹앱에서 무료로도 그대로 쓸 수 있습니다** — 잠긴 것은 창구이지 기능이 아닙니다.
3. **+ 메일 연동** — 어느 쪽이든 승인·수신거부·쿨다운·표현 규정·한도 게이트는 **똑같이**
   통과합니다. 잠기는 것이 있다면 전송 수단이지 발송 기능이 아닙니다.
   - **SMTP** (전 플랜) — Gmail·네이버·다음·아웃룩·회사 메일에서 직접 발송 → [`docs/MAIL-SETUP.md`](./docs/MAIL-SETUP.md)
   - **Gmail 연동** (**Agency 전용**) — '언론홍보' 라벨에 초안 자동 생성, 회신 모니터링 → [`docs/GMAIL-SETUP.md`](./docs/GMAIL-SETUP.md)
...
1. **기자 개인정보 보호** — 실명·이메일·연락처는 화면/출력에 **절대 표시하지 않는다**(익명 코드로 대체). 실제 이메일·실명은 **실제 발송 시점**에 수신자·본문으로만 사용한다. 자세히 → `docs/PRIVACY.md`
2. **본인 메일로만 발송** — 발송은 언제나 사용자 본인 계정에서 나간다. 기본은 SMTP(`docs/MAIL-SETUP.md`)이고, Agency 플랜은 Gmail **`언론홍보`** 라벨 워크플로우도 쓸 수 있다(`docs/GMAIL-SETUP.md`).
