...
3. **+ 메일 연동** — 둘 중 하나를 고릅니다. 어느 쪽이든 승인·수신거부·쿨다운·표현 규정·
   한도 게이트는 **똑같이** 통과합니다.
   - **Gmail** — '언론홍보' 라벨에 초안 자동 생성, 회신 모니터링 → [`docs/GMAIL-SETUP.md`](./docs/GMAIL-SETUP.md)
   - **SMTP** — 네이버·다음·아웃룩·회사 메일에서 직접 발송 → [`docs/MAIL-SETUP.md`](./docs/MAIL-SETUP.md)
...
2. **Gmail '언론홍보' 라벨** — Gmail을 쓴다면 모든 배포 메일과 회신을 **`언론홍보`** 라벨(그룹) 안에서 주고받는다. 설정 → `docs/GMAIL-SETUP.md` · Gmail이 아니라면 → `docs/MAIL-SETUP.md`
