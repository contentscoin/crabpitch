...
없었던 것이 가장 비싼 사고다. 전량 검토가 아니라 **1건**만 요구한다. 확정 경로 4곳 전부에
걸린다(아래 3차 후속 참조).
...
### 3차 후속 — Gmail 초안 경로의 게이트 우회 수정

①을 붙이다 드러난 것: Gmail 초안 생성 경로(`pushCampaignToGmail`)가 확정 로직을 따로 들고
있어 **7일 쿨다운·표현 규정·캠페인당 상한·월 한도를 통째로 건너뛰고** 있었다. 수신거부
재대조만 했다. Gmail 연결 사용자에게는 그게 기본 경로다.

원인은 순서다. 다른 세 경로는 선별과 확정을 한 트랜잭션에서 끝낼 수 있지만, Gmail은
중간에 외부 API 호출이 끼어 **선별 → 호출 → 확정**이라야 한다. 그 사정 때문에 경로가
자기 확정 로직을 복제했고, 복제된 쪽이 게이트를 잃었다.

`finalizeCampaignSend`를 두 단계로 쪼개 해결했다.

| 함수 | 하는 일 |
|---|---|
| `selectSendableDrafts` | 게이트 6종 전부. **아무것도 확정하지 않는다** |
| `confirmSent` | 초안을 sent로 바꾸고 사용량을 올린다. 이 파일에서 여기 한 곳뿐 |
| `finalizeCampaignSend` | 둘을 한 트랜잭션에서 (즉시·예약·크론) |
| `selectForGmailSend` / `confirmGmailSent` | 둘을 나눠서 (Gmail) |

- `gmailAccounts.markDraftsSentWithGmail` · `listPendingDraftsInternal` 삭제 — 두 번째 확정
  경로가 존재할 여지를 없앴다.
- 확정은 **멱등**이다. 외부 호출 뒤 확정이라 재시도로 같은 초안이 두 번 들어올 수 있고,
  그때 사용량이 이중 계상되면 안 된다.
- 외부 호출이 실패하면 초안은 그대로 남는다(확정 전이다).
- 제외분은 조용히 사라지지 않고 `제외: 7일 쿨다운 3건 · 표현 규정 1건` 형태로 알린다.
- 구조 가드를 확장해 **확정 코드가 두 번 나타나면 테스트가 깨진다**. 캠페인 상태 정리와
  초안 확정을 구분하려고 `status: "sent"` 단순 검색 대신 `sentAt` 동반 패턴으로 본다.
