...
12. **BYO AI (제품 핵심)** — 사용자 본인 ChatGPT·Claude·Gemini에서 공개 스킬/MCP로 보도자료·프레스킷 작성
    ...
    - **유저 MCP 키** — `userMcpKeys` + `/api/mcp` (`cp_mcp_…`). 발급은 Free 포함, 도구는 플랜별(`SKILL_ENTITLEMENTS`) — Free는 `crabpitch_press_guide`까지. 가이드 `docs/MCP-SETUP.md`
    - **프로덕션 반영(2026-07-25)** — `aromatic-shepherd-215`에 MCP HTTP 배포됨 (`/health` → `mcp:true`)
