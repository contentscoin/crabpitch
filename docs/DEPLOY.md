...
| `SMTP_ENCRYPTION_KEY` | **SMTP 발송을 쓰려면 필수.** 사용자 메일 비밀번호 봉인용 마스터 키. `openssl rand -base64 32` 출력을 그대로 넣습니다(base64 32바이트만 허용) |

#### `SMTP_ENCRYPTION_KEY` 주의

```bash
openssl rand -base64 32   # 이 출력을 그대로 Convex 환경변수에 넣습니다
```
